  ...
  def main(args: Array[String]): Unit = {
    ...
    val dfMap = Map(
      ...
      "lineitem" -> sc.textFile(inputDir + "/lineitem/*").map(_.split(separator)).map(p =>
        Lineitem(long(p(0)), long(p(1)), long(p(2)), long(p(3)), double(p(4)), double(p(5)), double(p(6)), double(p(7)),
          str(p(8)), str(p(9)), str(p(10)), str(p(11)), str(p(12)), str(p(13)), str(p(14)), str(p(15)))).toDF(),
    ...
    dfMap.foreach {
      case (key, value) =>
        value.write.format("parquet").option("compression", "none").mode(SaveMode.Ignore).save(outputDir + "/" + key)
    }
  }
}