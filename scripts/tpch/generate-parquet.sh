...
do
    ...
    mkdir -p $DB_UTILITIES/logs/gen/tpch
    ...
    then
        ...
        $SPARK_HOME/bin/spark-submit \
            --class benchmark.tpch.gen.ParquetGen \
            --master "local[*]" \
            --conf spark.driver.memory=$DRIVER_MEMORY \
            $JAR_PATH \
            $TBL_DIR $PARQUET_DIR $PARQUET_PARTITIONS > $DB_UTILITIES/logs/gen/tpch/s$SF-parquet.txt
    fi
done
